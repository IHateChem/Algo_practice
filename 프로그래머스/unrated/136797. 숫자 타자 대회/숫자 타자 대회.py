...
def solution(numbers):
    dp = [[MAX]*99 for _ in range(len(numbers)+1)]
    dp[0][46] = 0
    for i in range(1, len(numbers)+1):
        ...
        for j in range(99):
            ...
            l = j // 10
            r = j % 10
            if n != l and l*10 + n <99:
                dp[i][l*10+n] = min(dp[i][l*10+n], dp[i-1][j]+graph[r][n])
            if n != r and n*10+r<99:
                dp[i][n*10+r] = min(dp[i][n*10+r], dp[i-1][j]+graph[l][n])
    return min(dp[-1])
