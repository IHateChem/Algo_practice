...
def checkHistory(history, key, tot):
    if history[key] and history[key] <= tot: return 0
    history[key] = tot
    return 1
def solution(numbers):
    ...
    while 1:
        ...
        if idx == N: return tot
        n = numbers[idx]
        if r != n and checkHistory(history, (idx+1, n,r), tot + graph[l][n]):
            heapq.heappush(stack, (tot+graph[l][n], idx+1, n, r))
        if l != n and checkHistory(history, (idx+1, l,n), tot + graph[r][n]):
